# -*- coding: utf-8 -*-
"""
Created on Fri Aug  7 22:44:57 2020
...
#    base_num=2
    model = Sequential()
    model.add(Conv2D(32, (3, 3), input_shape=in_shape, padding='same', activation=actv))
...
#    model.add(Dropout(0.3))
    ...
    model.add(MaxPooling2D(pool_size=(2, 2)))
    model.add(Conv2D(256, (3, 3),  padding='same', activation=actv))
    model.add(Conv2D(512, (3, 3),  padding='same', activation=actv))
    model.add(Dropout(0.3))
...
#    lrate = 0.0001;decay = 0.007;momentum=0.3
#    optimizer = optimizers.SGD(lr=lrate, momentum=momentum, decay=decay, nesterov=False)
    optimizer=keras.optimizers.Adam(lr=0.0001, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=0.007)
